...
const EMPTY_FORM = {
  ...
  priceTable: {
    columns: ['Min Qty', 'Max Qty', 'Unit Price (AED)', 'Discount'],
    rows: [
      { id: '1', values: { 'Min Qty': '1', 'Max Qty': '49', 'Unit Price (AED)': '10.00', 'Discount': 'Standard' }, price: 10, isCardDisplayPrice: true },
      { id: '2', values: { 'Min Qty': '50', 'Max Qty': '199', 'Unit Price (AED)': '8.50', 'Discount': '15% OFF' }, price: 8.5, isCardDisplayPrice: false },
      { id: '3', values: { 'Min Qty': '200', 'Max Qty': '+', 'Unit Price (AED)': '7.00', 'Discount': '30% OFF' }, price: 7, isCardDisplayPrice: false },
    ],
  },
  ...
  features: [] as string[], benefits: [] as string[], applications: [] as string[],
  faqs: [] as { question: string; answer: string }[], contentGenerated: false,
  primaryKeyword: '', secondaryKeywords: [] as string[], semanticEntities: [] as string[]
};
...
export default function AdminProductForm() {
  ...
  const { products, categories, categoryDetails, addProduct, updateProduct, imagesSeo, updateImageSeo } = useStore();
  ...
  const [form, setForm] = useState<typeof EMPTY_FORM>(() => {
    if (existingProduct) {
      let loadedTable = EMPTY_FORM.priceTable;

      if (existingProduct.priceTable && existingProduct.priceTable.rows?.length > 0) {
        loadedTable = existingProduct.priceTable;
      } else if (existingProduct.priceTiers && existingProduct.priceTiers.length > 0) {
        loadedTable = {
          columns: ['Min Qty', 'Max Qty', 'Unit Price (AED)', 'Discount'],
      ...
      return {
        ...
        contentGenerated: existingProduct.contentGenerated || false,
        primaryKeyword: existingProduct.primaryKeyword || '',
        secondaryKeywords: existingProduct.secondaryKeywords || [],
        semanticEntities: existingProduct.semanticEntities || [],
      };
    }
    return EMPTY_FORM;
  });
  ...
  const categoryChain = React.useMemo(() => {
    ...
    return chain;
  }, [form.category, categories, categoryDetails]);
  ...
  // ── 100% Dynamic Price Table Handlers (Columns & Rows) ──
  ...
  const addPriceTableRow = () => {
    const table = form.priceTable || { columns: ['Option', 'Price (AED)'], rows: [] };
    const newRowId = Math.random().toString(36).substring(2, 9);
    const initialVals: Record<string, string> = {};
    table.columns.forEach(c => { initialVals[c] = ''; });
    const newRow = {
      id: newRowId,
      values: initialVals,
      price: 0,
  ...
  const updatePriceTableRowValue = (rowId: string, colName: string, val: string) => {
    const table = form.priceTable || { columns: [], rows: [] };
    const newRows = table.rows.map(r => {
      if (r.id !== rowId) return r;
      const newVals = { ...(r.values || {}), [colName]: val };
      let newPrice = r.price;
      if (/price|rate|cost|aed/i.test(colName)) {
        const num = parseFloat(val.replace(/[^0-9.]/g, ''));
        if (!isNaN(num) && num > 0) newPrice = num;
      }
      return { ...r, values: newVals, price: newPrice };
    });
  ...
  // ── Prompt / Smart Fill: Completely builds ONLY the columns & rows specified in the Prompt ──
  const parsePromptSmartFill = (rawText: string) => {
    ...
    // Check if input is standard range lines e.g. "1-49: 10, 50-199: 8.5" without a table header
    if (cleanLines.length >= 2 && !cleanLines[0].includes('|') && !cleanLines[0].includes(',') && !cleanLines[0].includes('\t') && /^\d+/.test(cleanLines[0])) {
      const columns = ['Quantity Range', 'Unit Price (AED)'];
      ...
      cleanLines.forEach((line, idx) => {
        ...
        if (rangeMatch) {
          const minQ = rangeMatch[1];
          const maxQ = rangeMatch[2];
          const p = parseFloat(rangeMatch[3]) || 0;
          rows.push({
            id: String(idx + 1),
            values: { 'Quantity Range': `${minQ} – ${maxQ} units`, 'Unit Price (AED)': p.toFixed(2) },
            price: p,
            isCardDisplayPrice: idx === 0,
          });
        } else if (openMatch) {
          const minQ = openMatch[1];
          const p = parseFloat(openMatch[2]) || 0;
          rows.push({
            id: String(idx + 1),
            values: { 'Quantity Range': `${minQ}+ units`, 'Unit Price (AED)': p.toFixed(2) },
            price: p,
            isCardDisplayPrice: idx === 0,
          });
        }
    ...
    // Structured Prompt with Columns (e.g. Thickness | Width | Length | Price)
    const columns = splitLine(headerLine).filter(Boolean);
    if (columns.length === 0) {
      alert('Could not detect column headers. Please provide columns separated by | or comma.');
      return;
    }
    ...
    rowLines.forEach((line, idx) => {
      ...
      columns.forEach((col, cIdx) => {
        ...
        values[col] = val;
        if (/price|rate|cost|aed/i.test(col) && rowPrice === 0) {
          const num = parseFloat(val.replace(/[^0-9.]/g, ''));
          if (!isNaN(num) && num > 0) rowPrice = num;
        }
      });

      // Fallback price detection across any cell if not in designated price column
      if (rowPrice === 0) {
    ...
    if (rows.length === 0) {
      alert('Could not parse any rows from your table input.');
      return;
    }

    // Completely replace table with prompt columns & rows (Zero extra forced columns!)
    set('priceTable', { columns, rows });
    setShowQuickTierModal(false);
    setQuickTierText('');
  };
  ...
  const handleGenerateAIContent = async () => {
    if (!form.name.trim() || !form.category) {
      alert('Product Name and Category are required to generate AI content.');
      return;
    }
    ...
    try {
      ...
      setForm(prev => ({
        ...
        semanticEntities: data.semanticEntities || prev.semanticEntities,
        contentGenerated: true
      }));

      // Highlight the AI generated fields so the admin knows what changed
      setHighlightedFields(['description', 'features', 'benefits', 'applications', 'specifications', 'faqs']);
      setTimeout(() => setHighlightedFields([]), 3000);

    } catch (err: unknown) {
      console.error(err);
      alert(err instanceof Error ? err.message : 'An error occurred during AI generation.');
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (form.priceType === 'fixed') {
      ...
      computedPrice = displayRow?.price || 0;
      computedMrp = form.priceTable?.rows?.[0]?.price || computedPrice;
      computedDiscount = 0;
    }
    ...
    const productData: Omit<Product, 'id'> = {
      ...
      ...(form.priceType === 'tiered' && form.priceTable ? { priceTable: form.priceTable } : {}),
      ...(form.priceType === 'tiered' && form.priceTiers && form.priceTiers.length > 0 ? { priceTiers: form.priceTiers } : {}),
      ...(form.priceType === 'range' && form.priceMin && form.priceMax ? {
      ...
      faqs: form.faqs,
      contentGenerated: form.contentGenerated,
      primaryKeyword: form.primaryKeyword.trim(),
      secondaryKeywords: form.secondaryKeywords,
      semanticEntities: form.semanticEntities,
  ...
  const renderCategorySelectors = () => {
    ...
    for (let i = 0; i <= categoryChain.length; i++) {
      const parentId = i === 0 ? null : categoryChain[i - 1];
      const availableOptions = categories.filter(c => (categoryDetails[c]?.parentId || null) === parentId);
      
      if (availableOptions.length === 0) break;

      const selectedValue = i < categoryChain.length ? categoryChain[i] : '';
      ...
      selectors.push(
        <div key={i} className={i > 0 ? 'mt-3' : ''}>
          ...
          <select
            ...
            className={`w-full border rounded-lg px-3.5 py-2.5 text-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-50 outline-none transition-all bg-white ${i === 0 && errors.category ? 'border-red-400 bg-red-50' : 'border-gray-300'} ${getHighlightClass('category')}`}
          >
            <option value="">{i === 0 ? 'Select Main Category...' : 'Select Sub-Category...'}</option>
            {availableOptions.map(c => <option key={c} value={c}>{c}</option>)}
          </select>
        </div>
      );
    }
            ...
            {activeTab === 'basic' ? (
              <>
                ...
                <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
                  ...
                  {/* 🆕 4th Option: 100% Dynamic Price Table (Fully customizable via Manual Edit or Prompt Takeover) */}
                      ...
                      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 bg-gradient-to-r from-blue-50/80 to-indigo-50/80 border border-blue-100 rounded-xl p-3.5">
                        <div>
                          <h3 className="text-xs font-extrabold text-blue-950 uppercase tracking-wider flex items-center gap-1.5">
                            ...
                            Dynamic Pricing & Specification Table
                          </h3>
                          <p className="text-[11px] text-slate-600 mt-0.5">
                            Add, edit, or delete any columns & rows. You can type column names directly in the header or use Prompt Fill!
                          </p>
                        </div>
                        <div className="flex items-center gap-2 self-stretch sm:self-auto shrink-0 flex-wrap">
                          ...
                          <button
                            ...
                            title="Add a custom column like Thickness, Width, Material, Packaging, etc."
                          >
                        ...
                        <div className="overflow-x-auto">
                          <table className="w-full text-left text-xs border-collapse">
                            <thead className="bg-slate-100/80 border-b border-slate-200 text-slate-700 font-extrabold uppercase tracking-wider text-[10px]">
                              <tr>
                                <th className="p-3 w-16 text-center border-r border-slate-200/80 bg-slate-100" title="Select which row's price is displayed on product cards">
                                  Card Price
                                </th>
                                
                                {/* Dynamic Editable Columns */}
                                {(form.priceTable?.columns || []).map((colName, cIdx) => (
                                  <th key={cIdx} className="p-2 border-r border-slate-200/80 min-w-[130px]">
                ...
                {/* ⚡ Quick-Fill / Prompt-Based Table Generator Modal */}
                {showQuickTierModal && (
                      ...
                      <div className="p-5 space-y-4">
                        <div>
                          ...
                          <textarea
                            ...
                            placeholder={`Thickness | Width | Length | Price (AED)\n0.13 mm | 12 mm | 10 m | 22.00\n0.13 mm | 25 mm | 10 m | 30.00\n0.13 mm | 50 mm | 10 m | 48.00\n0.18 mm | 50 mm | 10 m | 140.00\n0.13 mm | 50 mm | 30 m | 130.00`}
                        ...
                        <div className="bg-blue-50/60 border border-blue-100 rounded-xl p-3">
                          ...
                          <ul className="text-[11px] text-slate-600 space-y-0.5 list-disc list-inside">
                            <li><strong>Custom Columns:</strong> <code className="bg-white px-1 rounded border border-blue-100">Thickness | Width | Length | Price (AED)</code></li>
                            <li><strong>Quantity Tiers:</strong> <code className="bg-white px-1 rounded border border-blue-100">Min Qty | Max Qty | Unit Price (AED) | Discount</code></li>
                          </ul>
                          ...
                          <div className="mt-2.5 pt-2.5 border-t border-blue-100/80 flex items-center gap-2 flex-wrap">
                            ...
                            <button
                              type="button"
                              onClick={() => setQuickTierText(`Thickness | Width | Length | Price (AED)\n0.13 mm | 12 mm | 10 m | 22.00\n0.13 mm | 25 mm | 10 m | 30.00\n0.13 mm | 50 mm | 10 m | 48.00\n0.18 mm | 50 mm | 10 m | 140.00\n0.13 mm | 50 mm | 30 m | 130.00`)}
                            ...
                            >
                              Thickness / Dimensions (5 rows)
                            </button>
                            <button
                              type="button"
                              onClick={() => setQuickTierText(`Min Qty | Max Qty | Unit Price (AED) | Discount\n1 | 49 | 10.00 | Standard\n50 | 199 | 8.50 | 15% OFF\n200 | + | 7.00 | 30% OFF`)}
                              className="text-[11px] bg-white border border-blue-200 text-blue-700 px-2 py-0.5 rounded hover:bg-blue-50 font-medium cursor-pointer"
                            >
          ...
          <div className="space-y-5">
            {activeTab === 'basic' ? (
              ...
              <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5 space-y-4 sticky top-6">
                ...
                {(() => {
                  const generatedSeo = generateProductSEO(form as unknown as Product);
                  const displayTitle = generatedSeo.title;
                  const displayDesc = generatedSeo.description;
