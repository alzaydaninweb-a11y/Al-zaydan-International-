...
export default function PriceDisplay({ product, size = 'sm' }: Props) {
  ...
  if (type === 'hidden') {
    const WAIcon = () => (
      <svg viewBox="0 0 24 24" className={isLarge ? 'w-5 h-5' : 'w-3 h-3'} fill="currentColor">
    ...
    return isLarge ? (
      <div className="inline-flex items-center gap-2.5 bg-amber-500/10 border border-amber-500/30 text-amber-800 font-extrabold px-4 py-2.5 rounded-xl text-base shadow-sm">
        <WAIcon />
        Price on Request / Get Custom Quote
      </div>
    ) : (
  ...
  if (type === 'range') {
    ...
    return isLarge ? (
      <div className="flex flex-wrap items-baseline gap-2">
        <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">B2B Volume Range:</span>
        <div className="flex items-baseline gap-1.5">
          <span className="text-sm font-bold text-blue-600">AED</span>
          <span className="text-3xl sm:text-4xl font-black text-slate-900 tracking-tight">{min.toLocaleString()}</span>
          <span className="text-xl font-bold text-slate-400 mx-1">–</span>
          <span className="text-sm font-bold text-blue-600">AED</span>
          <span className="text-3xl sm:text-4xl font-black text-slate-900 tracking-tight">{max.toLocaleString()}</span>
        </div>
      </div>
    ) : (
  ...
  return isLarge ? (
    <div className="flex flex-wrap items-center gap-3">
      <div className="flex items-baseline gap-1.5">
        <span className="text-sm font-extrabold text-blue-600">AED</span>
        <span className="text-3xl sm:text-4xl font-black text-slate-900 tracking-tight">{priceVal.toFixed(2)}</span>
      </div>

      {hasDiscount && (
        <span className="text-base text-slate-400 line-through font-semibold">
          AED {mrpVal.toFixed(2)}
        </span>
      )}

      {discountPercent > 0 && (
        <span className="bg-emerald-500 text-white font-black text-[11px] px-2.5 py-1 rounded-full uppercase tracking-wider shadow-sm">
